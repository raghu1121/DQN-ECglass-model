...
output = 'Output_preprocessing_dgps/'
climates = {
...
for climate in climates:
    dfs = []
    EC_files = sorted(glob.glob(input+'DGPs_EC/' + climates[climate]['code_name'] + '/*'))
    ...
    comb_dfs.to_csv(output+'S_env_comb_' + climates[climate]['name'] + '.csv', index=False)
    env_dfs = comb_dfs[
    ...
    env_dfs.to_csv(output+'S_env_' + climates[climate]['name'] + '.csv', index=False)
    obs_dfs = comb_dfs[['altitude', 'azimuth', 'dir', 'diff', 'Occupancy']]
    obs_dfs = obs_dfs.drop_duplicates()
    obs_dfs.to_csv(output+'S_observations_' + climates[climate]['name'] + '.csv', index=False)
    # print(comb_dfs['Time'])
